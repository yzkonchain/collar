...
export default function Borrow() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const _bond = ethers.utils.parseUnits(state.I.bond || '0', 18)
      const _want = await controller.ct(pool, signer).get_dy(_bond)
      ...
      if (_bond.eq(state.input.bond) === false) {
        setState({ input: { bond: _bond }, output: { want: _want }, tip: { fee, min, slip } })
      }
    })()
  }, [state])

  useEffect(() => {
    setState({ I: { bond: '' }, tip: { fee: '0.0000', min: '0.000', slip: '0.00' }, old: { bond: '' } })
  }, [pool])
          ...
          <div>
            <AmountInput
              ...
              State={{
                ...
                setState,
                token: bond,
                max: parseFloat(ethers.utils.formatEther(data.balance.bond)),
        ...
        <div className={classes.button}>
          <div>
            <MyButton
              name="Approve"
              onClick={async () => {
                await controller.approve(bond, pool, signer)
            ...
            <MyButton
              name="Deposit & Borrow"
              onClick={async () => {
                await controller.borrow(state.input.bond, state.output.want, pool, signer)
                setLiteState({ forceUpdate: {} })
              }}
            />
          </div>
